...
def fantasy(request):
    ...
    game_logs_qs = RawGameLog.objects.filter(season_year=season_year).values_list(
        'player_id', 'player_name', 'season_year', 'min', 'pts', 'fg3m', 'reb', 'ast', 'stl', 'blk', 'fgm', 'fga', 'ftm', 'fta', 'tov', 'usg_pct'
    )
    game_logs_df = pd.DataFrame(
        list(game_logs_qs),
        columns=['player_id', 'player_name', 'season_year', 'min', 'pts', 'fg3m', 'reb', 'ast', 'stl', 'blk', 'fgm', 'fga', 'ftm', 'fta', 'tov', 'usg_pct']
    )
    averages_df = game_logs_df.groupby(['player_id', 'player_name', 'season_year']).agg({
        'min': 'mean',
        'pts': 'mean',
        'fg3m': 'mean',
        'reb': 'mean',
...
def fantasy_matchup(request):
    ...
    game_logs_qs = RawGameLog.objects.filter(season_year=season_year, player_name__in=rostered_players).values_list(
        'player_id', 'player_name', 'season_year', 'min', 'pts', 'fg3m', 'reb', 'ast', 'stl', 'blk', 'fgm', 'fga',
        'ftm', 'fta', 'tov', 'usg_pct'
    )
    game_logs_df = pd.DataFrame(
        list(game_logs_qs),
        columns=['player_id', 'player_name', 'season_year', 'min', 'pts', 'fg3m', 'reb', 'ast', 'stl', 'blk', 'fgm',
                 'fga', 'ftm', 'fta', 'tov', 'usg_pct']
    )
    averages_df = game_logs_df.groupby(['player_id', 'player_name', 'season_year']).agg({
        'min': 'mean',
        'pts': 'mean',
    ...
    for team in (team_1_df, team_2_df):
        ...
        cols = ['player_name', 'min', 'pts', 'fg3m', 'reb', 'ast', 'stl', 'blk', 'fgm',
                   'fga', 'fg_pct', 'ftm', 'fta', 'ft_pct', 'tov', 'games_played']
        team = team[cols]
    ...
    return render(request, 'fantasy/matchup.html', {
        'team_1': team_1,
        'team_2': team_2,
    })
